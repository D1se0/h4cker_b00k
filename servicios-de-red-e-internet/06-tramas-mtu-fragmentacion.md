...
> 📌 _Nota de tu apunte:_ "Tamaño máximo de transmisión por una trama, ¿1500?" — Sí: el **payload** de la trama Ethernet estándar es 1500 bytes (la MTU de IP). La trama completa puede llegar a 1518 con cabeceras, o **\~1514 bytes útiles en modo promiscuo** según el intérprete, y con jumbo frames (9000) en redes de storage/DC.
...
* Si el paquete no cabe en un enlace y lleva DF puesto → **se descarta** y el router devuelve un **ICMP Type 3 Code 4** ("fragmentation needed and DF set", _Packet Too Big_ en IPv6).
...
> 📌 _De tu apunte:_ "La segmentación numera, la fragmentación no." Exactamente: si se pierde un segmento TCP, se retransmite solo ese; si se pierde un fragmento IP, **el paquete entero se reenvía** (por eso fragmentar es caro y se evita).
...
> 📌 _De tu apunte:_ "Si mando entera una trama, el cable se dedicaría únicamente a esa trama gigante".
...
> 📌 _Definiciones cortas para el examen:_
>
