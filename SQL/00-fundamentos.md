# 🗄️ Capítulo 0 — Fundamentos: Bases de Datos y SQL
...
> 💡 **Dato clave para hacking:** SQL es un lenguaje **declarativo**: tú dices *qué* quieres, no *cómo* hacerlo. Esto significa que las consultas se construyen como frases: `SELECT ... FROM ... WHERE ...`. Entender esta estructura es lo que te permitirá entender las inyecciones SQL del capítulo 7.
...
`NULL` significa "**sin valor**". No es `0` ni una cadena vacía: es *ausencia de dato*. Se estudia a fondo en el capítulo 3.
...
🎉 Eso es SQL: "*dame todas las columnas (`*`) de la tabla `patients`*". Si has visto resultados, ya estás haciendo SQL.
